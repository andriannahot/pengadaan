                ...
                <table  style="text-align: center;" class="table table-hover text-nowrap">
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td><a target="_blank" href="{{$p->deskripsi}}"><button class="btn btn-primary">Lihat Detail</button> </a></td>
                      <td style=" width:15%">
                        @if ($p -> gambar != "-")
                        <img style="width: 70%;" src="{{asset(Storage::url($p->gambar))}}"><hr>
                        <a class="konfirmasi" href="/hapusGambar/{{$p->id_pengadaan}}"><button class="btn btn-danger"><i class="fas fa-trash"> Hapus</button></a></i>
                          @endif
                      ...
                      <td><span class="tag tag-sucecss">{{number_format($p->anggaran,0,",",".")}}</span></td>
                      <td><a href="/hapusPengadaan/{{$p->id_pengadaan}}" class="konfirmasi"> <button class="btn btn-danger"><i class="fas fa-trash"> Hapus</button></i></a>
                        <button data-id_pengadaan="{{$p->id_pengadaan}}" data-nama_pengadaan="{{$p->nama_pengadaan}}" data-deskripsi="{{$p->deskripsi}}" data-anggaran="{{$p->anggaran}}" data-toggle="modal" data-target="#ubahModal" class="btn btn-secondary ubah"><i class="fas fa-edit"> Ubah</i></button>
                      </td>
                    </tr>
                    @endforeach
                  </tbody>
...
$(document).on("click", ".konfirmasi", function(event){
    ...
    var answer = window.confirm("Kamu Yakin Menghapus Data Ini...??");
